...
const ResultDescription = ({
  ...
  const renderPeriodText = () => {
    if (interval) {
      return (
        <>
          {' '}
          from <strong>{formatUnix(period[0])}</strong> to{' '}
          <strong>{formatUnix(period[0])}</strong>
        </>
      )
    } else if (!forever) {
      return (
        <>
          {' '}
          in the past{' '}
          <strong>{(PERIODS.get(period) as string).toLowerCase()}</strong>
        </>
      )
    }
  }
  ...
  return (
    <Container>
      <span>
        <strong>{dimensions}</strong> collage of top <strong>{typeName}</strong>{' '}
