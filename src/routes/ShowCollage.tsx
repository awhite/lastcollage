...
const ShowCollage = ({ collageGenerationParams }: RouteProps) => {
  ...
  const { imgUrl, downloadPath, errorMessage, rowNum, colNum } =
    location.state as ShowCollageState
  ...
  return (
    <InputScreen>
      <ResultDescription
        {...(collageGenerationParams as CollageGenerationParams)}
        colNum={colNum}
        rowNum={rowNum}
      />
      <CollageDisplay
