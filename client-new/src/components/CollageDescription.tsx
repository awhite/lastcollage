...
import { CollageGenerationParams, CollageType, DateInterval, FixedPeriod, Period } from '../types'
...
const formatFixedPeriod = (fixedPeriod: FixedPeriod) => (PERIODS.get(fixedPeriod) as string).toLowerCase()

const formatPeriod = (period: Period) =>
  isDateInterval(period) ? formatDateInterval(period as DateInterval) : formatFixedPeriod(period as FixedPeriod)
const formatType = (type: CollageType) => (TYPES.get(type) as string).toLowerCase()
