...
const Welcome = ({ isMaintenanceMode }: Props) => {
  const [lastCollageInfo, setLastCollageInfo] = useState<CollageGenerationParams | null>(null)

  useEffect(() => {
    const serializedLastCollage = localStorage.getItem('lastcollage')
    if (!serializedLastCollage) return

    const lastCollageGenerationParams = JSON.parse(serializedLastCollage) as CollageGenerationParams
